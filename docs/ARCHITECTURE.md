...
- MergeView 与轻量 CodeMirror 共用受控滚动同步：捕获阶段的非被动滚轮监听先于 CodeMirror 内置滚轮预处理执行，并阻止同一事件继续传播，只推动当前来源栏的可见滚动条。普通 `scroll` 事件通过同一个 `requestAnimationFrame` 槽位合并，积压输入只保留最新来源位置；同一次连续滚动会固定真实操作栏，每个目标栏的程序滚动期望队列及来源锁共同阻止迟到事件反向驱动原栏。其他栏在普通拖动期间每帧最多跟随一次，每次输入仍重置 `200 ms` 计时器，稳定后按最终比例或行对齐位置再校准一次。MergeView 保留同步滚动开关，三栏按滚动比例保持相同文档进度；关闭或切换时解除监听并取消计时器。
- 普通历史文件进入 `compareMode === "align"` 时，两个只读 CodeMirror 使用 `viewportMargin: Infinity` 一次性创建门限内的全部行和 spacer，避免快速滚动时虚拟视口、行高测量与对齐 spacer 互相触发重排。该边界只适用于已经通过体积、行数和差异复杂度保护的历史行对齐；连线模式、工作区、冲突和轻量路径继续使用有界视口或轻量编辑器。
