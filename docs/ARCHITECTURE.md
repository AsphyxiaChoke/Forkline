...
- 工作区与历史文件共用复杂度判断：内容达到 `768 KiB`、行数达到 `20,000`，或连续差异达到 `2,000` 行/`32` 个区段时，在创建 `MergeView` 前直接进入轻量双栏 CodeMirror；普通小文件仍保留完整对照和暂存能力。该前置判断覆盖“文件未超过 `1 MiB` 但内容较大或行数很多”的工作区文件，避免首次双击期间先同步构造完整差异视图。
...
- 普通 MergeView 与轻量编辑器共用受控滚动同步：每个目标栏维护程序滚动期望队列，明确滚轮输入在短活动窗口内作为唯一用户来源，实际滚动事件只从活动来源驱动同步，滚轮停止后再执行一次最终收敛；程序滚动和 CodeMirror 内部位置校正不能反向驱动原栏。MergeView 保留原同步滚动开关；三栏按左右锁定关系决定可达面板，行对齐重算在清理 spacer 前保存各栏位置。滚动条、键盘和普通程序化滚动仍由原生 `scroll` 路径同步，关闭或切换时解除监听并取消待执行帧。
