...
- 当前完整自动回归为 `122/122`：在实时命令与输出、运行状态接口、可取消动作边界、Windows 进程树终止、取消响应与日志状态，以及克隆弹窗和操作日志布局回归基础上，新增历史文件“连线 / 行对齐”切换及大文件保持轻量路径的前端回归。
...
- 历史提交完整文件对照继续复用同一浮窗：`GET /api/commit-file?sha=&file=&previousFile=` 返回当前提交和第一父提交中的完整文本，重命名读取旧路径，根提交没有父版本，merge 提交与第一父提交比较。提交“文件”页单击只更新选中状态并保持文件树 DOM 与滚动位置，双击才打开完整对照；小文件保持只读 MergeView 和查找，顶部可切换“连线 / 行对齐”：连线保留原差异连接，行对齐使用 CodeMirror spacer 在增删行另一侧补空白，使后续相同行水平对齐，选择在当前页面会话中沿用。大文件继续使用上述双 CodeMirror 轻量模式且不显示此切换，冲突和工作区编辑器也不受影响。接口支持到 `16 MiB` 的 UTF-8、UTF-8 BOM、GBK、GB18030 文本，超过上限或二进制文件会给出中文错误。
...
- `tests/backend-modules.test.js` 固定两层模块边界，`tests/backend-services.test.js` 直接覆盖补丁、路径、认证 URL、恢复策略、分支保护和历史分页；完整 `npm test` 当前为 `139/139`，真实 Git 回归继续覆盖文件编辑、GBK/GB18030、储藏、分支、合并、历史改写、冲突、同步和取消操作。
