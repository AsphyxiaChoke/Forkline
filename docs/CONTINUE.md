...
- 引用日志恢复已接入：右侧“恢复点”页新增 HEAD reflog 区块，读取 `git log -g HEAD`，可查看 reflog 提交、从某条 reflog 创建 Forkline 恢复点，或在本地分支且工作区干净时恢复到该记录；恢复到 reflog 记录会携带页面看到的当前分支和 HEAD，避免旧页面把外部切换后的分支硬重置到错误提交；reflog 行右键菜单也提供“查看提交 / 复制 SHA / 创建恢复点 / 恢复到此处”；reflog 字段改用 NUL 分隔，提交标题里包含内部控制分隔符时不会把作者和时间解析错位。
