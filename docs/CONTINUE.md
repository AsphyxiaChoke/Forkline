...
- 当前分支类写操作已加页面快照保护：暂存全部、丢弃全部、提交、追加提交、拉取、变基拉取、推送、安全强推、合并、分支变基、挑选、还原、reset、修改提交信息和历史编辑会携带页面看到的当前分支和 HEAD；如果仓库已被外部命令切到其他分支，或当前分支 HEAD 已经移动，后端会拒绝旧页面请求，避免把操作打到错误分支或旧页面之外的新提交；旧脚本缺少快照时也会被拒绝。
...
- 同步页远端仓库管理已接入：显示 `git remote -v` 的 fetch / push URL，支持添加远端、抓取单个远端、修改 URL、删除远端；远端行右键菜单也提供抓取、修改 URL、复制 URL、删除远端，并显示对应 Git 指令；远端名允许 Git 合法的带 `/` 名称，例如 `team/origin`。
