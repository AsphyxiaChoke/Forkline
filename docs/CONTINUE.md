...
- 弹窗式工作区文件编辑器已升级为 `HEAD` / 工作区双栏：左栏只读、右栏可编辑，重命名文件会携带 `previousFile` 对照旧路径，未跟踪文件左栏为空；CodeMirror MergeView 提供行号、旧版本红色 / 新版本绿色的块级及词级差异强调、C/C++、JavaScript、TypeScript、HTML、CSS、Python、Markdown 等常见模式高亮，中间箭头可将单个变更块回写为旧版本，回写只修改右栏未保存内容，不自动保存或暂存；桌面弹窗最大使用 `1920×1200` 且贴近当前视口，可拖动标题栏空白区域移动，并通过右下角手柄调整大小，移动和缩放会限制在当前视口内，`ResizeObserver` 会在尺寸变化后刷新双栏和差异连线；中间操作区固定 `48px`，窄屏固定 `40px`，窄屏同时禁用自由拖动和缩放并保持近全屏，左右栏继续各自横向滚动而不强制代码换行；并内置查找、上一处、下一处、区分大小写、单处替换和全部替换，快捷键为 `Ctrl+F`、`Ctrl+H`、`Ctrl+S`。保存后继续选中原文件并刷新未暂存 Diff，不自动暂存，切换仓库会销毁编辑器实例。后端 `/api/worktree-file` 支持不超过 1 MiB 的 UTF-8、UTF-8 BOM、GBK 和 GB18030 普通文本，保存时保持原编码及 CRLF / LF / CR，并以 SHA-256 快照阻止外部修改后的旧内容覆盖；二进制、符号链接、目录、任意层级 `.git` 路径和越界路径会被拒绝。浏览器资源、iconv-lite 及其运行依赖 safer-buffer 直接随仓库提供，启动仍不需要安装 npm 依赖；`tests/portable-runtime.test.js` 会检查这些依赖文件已被 Git 跟踪，并在清空 `NODE_PATH` 后验证 GBK 编解码。
