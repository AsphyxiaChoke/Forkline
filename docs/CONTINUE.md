...
- 工作区 Diff 已支持按行选择暂存/取消暂存：底部 Diff 和最大化 Diff 中都可选择新增/删除行；未暂存视图显示“暂存所选行”，已暂存视图显示“取消暂存所选行”，修改行会按成对增删一起处理；暂存后会自动切到已暂存视图，暂不包含行内容编辑。
