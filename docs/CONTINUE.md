...
- 当前分支类写操作已加页面快照保护：本地/远端分支签出、新建分支、重命名分支、暂存全部、单文件暂存/取消暂存/丢弃、加入 `.gitignore`、冲突文件取舍、工作区 Diff 按块/按行操作、丢弃全部、创建工作树、初始化/更新/同步子模块、创建/恢复/应用/弹出/删除储藏、从储藏创建分支、提交、追加提交、拉取、变基拉取、推送、安全强推、设置或取消 upstream、应用补丁、合并、分支变基、挑选、还原、reset、恢复点恢复、引用日志恢复、修改提交信息、历史编辑，以及合并/变基/挑选/还原的继续、跳过、中止操作会携带页面看到的当前分支和 HEAD；如果仓库已被外部命令切到其他分支，或当前分支 HEAD 已经移动，后端会拒绝旧页面请求，避免把操作打到错误分支或旧页面之外的新提交；拉取、变基拉取、推送、安全强推和取消 upstream 还会校验页面看到的 upstream 及相关远端 URL，没有 upstream 的智能推送会校验页面看到的默认推送远端；旧脚本缺少快照时也会被拒绝。
