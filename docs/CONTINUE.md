...
- 当前完整自动回归为 `147/147`：在实时命令与输出、运行状态接口、可取消动作边界、Windows 进程树终止、取消响应与日志状态，以及克隆弹窗和操作日志布局回归基础上，覆盖历史文件“连线 / 行对齐”切换、大文件与复杂差异轻量路径、旧 MergeView 即时释放和真实 Chromium 性能边界。
...
2. 远端同步体验继续补：同步摘要、force-with-lease、远端 URL 管理、upstream 管理、推送前分叉保护、变基拉取、同步提交预览、诊断向导、按需认证环境检测和轻量 `/api/sync-state` 刷新已完成；后续可继续做系统凭据管理器的操作入口和远端托管平台状态检查。
3. 恢复点策略继续增强：现在已有危险历史操作完成后的顶栏一键撤销、手动策略清理、本地偏好记忆和清理前候选列表展开；后续可以增加危险操作完成后可选自动执行保留策略，以及按仓库单独保存策略。
