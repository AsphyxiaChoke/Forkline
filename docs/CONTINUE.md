...
- 当前分支类写操作已加页面快照保护：本地/远端分支签出、新建分支、暂存全部、单文件暂存/取消暂存/丢弃、加入 `.gitignore`、冲突文件取舍、工作区 Diff 按块/按行操作、丢弃全部、创建工作树、初始化/更新/同步子模块、创建/恢复/应用/弹出/删除储藏、从储藏创建分支、提交、追加提交、拉取、变基拉取、推送、安全强推、设置或取消 upstream、应用补丁、合并、分支变基、挑选、还原、reset、恢复点恢复、引用日志恢复、修改提交信息、历史编辑，以及合并/变基/挑选/还原的继续、跳过、中止操作会携带页面看到的当前分支和 HEAD；如果仓库已被外部命令切到其他分支，或当前分支 HEAD 已经移动，后端会拒绝旧页面请求，避免把操作打到错误分支或旧页面之外的新提交；拉取、变基拉取、推送、安全强推和取消 upstream 还会校验页面看到的 upstream 及相关远端 URL，没有 upstream 的智能推送会校验页面看到的默认推送远端；旧脚本缺少快照时也会被拒绝。
...
- 分支整理删除策略：后端新增 `deleteBranches`，批量删除仍逐个执行 `git branch -d`；当前分支、`main` / `master` / `develop` / `dev` / `trunk` 这类主干分支、以及其他 worktree 占用分支会在 UI 禁用；后端也会拒绝删除当前分支和删除/重命名这些主干分支，未完全合并的分支仍由 Git 拦截并返回中文原因；本地分支删除、批量删除和重命名都会携带页面看到的分支 HEAD SHA，后端执行前会确认同名分支仍指向同一提交，避免旧页面把外部移动后的新分支改名或删除。
...
- Git submodule 管理已接入：右侧新增“子模块”页，读取 `.gitmodules` 和 `git submodule status --recursive`，显示子模块路径、URL、目标提交、初始化状态、提交不一致和本地改动数量；大量子模块会完整返回，不再只显示前 80 个；支持初始化全部、更新全部、更新单个子模块、同步 URL、复制路径和复制 URL；子模块写操作会携带页面看到的当前分支和 HEAD，避免旧页面按外部切换后的分支记录更新子模块工作区或同步 URL。
...
- 自动恢复点已接入：追加提交、变基拉取、分支变基、修改提交信息、交互式历史编辑和 reset 前会自动创建 `refs/forkline/recovery/...` 隐藏引用；同一秒内同一分支连续创建同类恢复点时，后续恢复点会追加 `-2`、`-3` 后缀，避免覆盖更早的恢复点；右侧新增“恢复点”页，可查看、恢复或删除恢复点。恢复动作会先再创建一个恢复前恢复点；恢复会携带页面看到的当前分支、HEAD 和恢复点 SHA，后端发现分支已切换、HEAD 已移动或同一 ref 已经指向其他提交时会拒绝旧页面请求；删除会校验页面看到的恢复点 SHA。
