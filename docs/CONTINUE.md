...
- 设置界面已接入：顶部右上“更多”下拉和命令面板都能打开“设置”，目前集中管理本机主题、界面语言、最近仓库、恢复点保留策略和布局宽高重置；默认使用中文，可切换为英语并保存在当前浏览器本地，不改仓库配置。界面、弹窗、提示、操作日志、诊断信息、后端错误和相对时间均支持中英双语，仓库路径、分支名、标签、提交信息、作者、引用、提交哈希、命令及原始 Git 输出保持原值。
...
- 工作区 Diff 已支持按行选择暂存/取消暂存：底部 Diff 和最大化 Diff 中都可选择新增/删除行；未暂存视图显示“暂存所选行”，已暂存视图显示“取消暂存所选行”，修改行会按成对增删一起处理；已暂存的新文件也支持只取消暂存选中行，工作区已删除文件也支持只暂存选中的删除行，未选行会留在另一侧 Diff 中；代码行超过可视宽度时会保留完整内容并提供水平滚动；读取 Diff 补丁时只使用 Git stdout，避免 Windows CRLF warning 混入补丁导致按行操作失败；暂存后会自动切到已暂存视图，暂不包含在 Diff 行内直接编辑。
...
- 编辑器中间块暂存已改为编辑器专用零上下文 Diff：`/api/worktree-file` 返回 `diffContext = 0`，前端用 MergeView 回调提供的旧版和新版范围共同定位 hunk，并把上下文随 `stageHunk` 传回后端；后端使用同一上下文重新生成补丁，并在零上下文时给 `git apply` 增加 `--unidiff-zero`。因此两个视觉块即使在普通 8 行上下文 Diff 中会合并，点击其中一个也只暂存该块；纯删除块仍可通过旧版范围准确定位。底部工作区 Diff 继续使用原有 8 行上下文，不受影响。
...
- 右侧提交“文件”页的 Diff 已针对窄检查器改为单列阅读：上下文行只显示一份，修改行按旧版删除在上、新版增加在下排列，长代码仍可水平滚动；文件名、路径与“文件历史 / 逐行追踪 / 最大化”操作分成两行，避免按钮和标题互相挤压。底部工作台和最大化 Diff 继续保留旧版/新版双栏对照。
