...
- 图谱分支标签已改为完整显示：移除原先 7 到 8 个字符的省略规则，按完整分支名和所在泳道计算图谱列宽及 SVG 宽度；图谱表头中的“全部分支”或当前分支名也固定按内容宽度显示，不再参与 flex 收缩或使用省略号。`feature/visual-history`、`fix/diff-pane-resize`、`release/2.9`、`origin/main` 等标签实测完整显示。
...
- 历史提交完整文件对照已接入：右侧提交“文件”页的变更文件双击后复用现有浮窗和 CodeMirror MergeView，`GET /api/commit-file?sha=&file=&previousFile=` 返回当前提交和第一父提交中的完整文本；重命名使用 `previousFile` 读取旧路径，根提交没有父版本，merge 提交固定与第一父提交比较。新增、删除和一侧不存在时仍保留完整另一侧内容及差异占位。提交文件选择改为只替换 `.commit-file-diff`，不再重建整个详情页；因此大量文件列表的 DOM、滚动位置和双击监听会保留，单击下方文件后仍可原地继续双击。打开历史浮窗时，右侧 `.commit-file-diff` 会暂停重型行渲染并改成轻量提示，已移除的底部 `workDiffView` 只保留 `activeDiff` 数据供“最大化”使用，不再生成隐藏副本；浮窗打开期间切换提交文件仍保持暂停，关闭后恢复当前选中文件 Diff。只读历史模式也不会创建仅供暂存按钮定位的 `MutationObserver`。该模式通过独立 `history` 状态严格只读，只保留行号、语法高亮、红绿差异、连接线、同步滚动和查找，隐藏保存、替换、暂存按钮及编辑器 Git 右键菜单；只读模式将 MergeView 的 `connect` 设为普通连接线，不启用 `align` 的强制垂直行对齐，避免特定文件触发持续布局计算，工作区模式仍保留 `align` 和按块暂存按钮；再打开工作区文件时会完整恢复可编辑状态。接口继续拒绝非法相对路径、二进制和超过 1 MiB 的文件，并按 UTF-8、UTF-8 BOM、GBK、GB18030 解码。
- 历史文件右侧 Diff 与最大化 Diff 已增加大差异分批渲染：初始上限为 `1000` 条原始 Diff 行，“继续加载”每次把上限翻倍直到完整显示；`activeDiff` 始终保留完整数据，不影响最大化、按文件切换或后续操作。右侧预览按“提交 SHA + 文件路径”记录批次，切换文件后回到首批；重新渲染更多行时恢复原滚动位置；关闭最大化窗口会 `replaceChildren()` 清空隐藏 DOM。
...
- 提交详情聚合 Diff 预览已移除：点击提交后的右侧“详情”页只保留元信息、提交信息和相关操作，不再渲染整条提交的 Diff；完整 Diff 数据、“文件”页按文件对照和最大化查看继续保留。
- Diff 行文本不再截断到 280 字符：长配置行、长 JSON 行或长路径相关元信息会完整返回给前端，避免对照里看不到行尾真实改动。
- 右侧提交“文件”页的 Diff 已针对窄检查器改为单列阅读：上下文行只显示一份，修改行按旧版删除在上、新版增加在下排列，长代码仍可水平滚动；文件名、路径与“文件历史 / 逐行追踪 / 最大化”操作分成两行，避免按钮和标题互相挤压。最大化 Diff 继续保留旧版/新版双栏对照。
