...
- 安全强推已接入：顶部工具栏新增“强推”，当前本地分支右键菜单新增“安全强推当前分支”，后端执行 `git push --force-with-lease <远端> HEAD:<分支>`；普通 push 非快进拒绝和 force-with-lease stale 拒绝都有中文提示；后端会拒绝把安全强推打到 `main` / `master` / `develop` / `dev` / `trunk` 这类远端主干/长期分支。
...
- 同步页远端仓库管理已接入：显示 `git remote -v` 的 fetch / push URL，支持添加远端、抓取全部远端、抓取单个远端、修改 URL、删除远端；远端行右键菜单也提供抓取、修改 URL、复制 URL、删除远端，并显示对应 Git 指令；远端名允许 Git 合法的带 `/` 名称，例如 `team/origin`；抓取全部远端会携带页面看到的远端列表和 fetch/push URL，抓取指定远端、修改 URL 或删除远端会携带页面看到的单个远端 fetch/push URL，后端发现远端 URL 已被外部命令改过时会拒绝旧页面请求，避免旧页面覆盖、删除或抓取新的远端配置。
...
- Git worktree 管理已接入：右侧新增“工作树”页，读取 `git worktree list --porcelain` 并对存在的工作树补充干净/有改动摘要；大量 worktree 会完整返回，不再只显示前 24 个；支持从任意引用创建工作树、可选同时创建新分支、打开已有工作树、复制路径、刷新列表和执行 `git worktree prune --verbose` 清理失效记录；创建工作树会携带页面看到的当前分支和 HEAD，避免旧页面的 `HEAD` 起点在外部切换分支后从错误提交创建工作树。
