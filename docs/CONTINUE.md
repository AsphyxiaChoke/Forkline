...
- 当前完整自动回归为 `152/152`：在实时命令与输出、运行状态接口、可取消动作边界、Windows 进程树终止、取消响应与日志状态，以及克隆弹窗和操作日志布局回归基础上，覆盖认证助手系统凭据入口、托管平台识别、历史文件“连线 / 行对齐”切换、大文件与复杂差异轻量路径、旧 MergeView 即时释放和真实 Chromium 性能边界。
...
- 默认 `npm test` 已加入零额外依赖的真实 Chromium 性能回归：自动寻找 Edge、Chrome 或 Chromium，通过 DevTools Protocol 启动无头浏览器、随机端口 Forkline 服务和独立临时 Git 仓库，验证复杂历史文件打开、事件循环、同步滚动、关闭释放，以及普通 MergeView 连续切换后的全局监听器数量。普通完整测试在缺少浏览器时会标记跳过；`npm run test:browser` 或 `FORKLINE_REQUIRE_BROWSER=1` 会把缺少浏览器视为失败，`FORKLINE_BROWSER_PATH` 可指定自定义路径。
