...
- 远端分支右键菜单已接入“删除远端分支”，后端执行 `git push <远端> --delete <分支>` 并随后 `fetch --prune`；无效远端引用不会给出删除入口；后端会拒绝删除 `main` / `master` / `develop` / `dev` / `trunk` 这类远端主干/长期分支。
...
- Tag 管理已接入：右侧新增“标签”页，显示本地 Tag 列表和详情，支持查看 Tag 提交、复制名称、推送 Tag、删除本地 Tag、删除远端 Tag；Tag 说明里包含 Tab 时不会把说明和对象类型解析错位；Tag 行右键菜单也提供同样的相关动作和 Git 指令提示。
...
- 自动恢复点已接入：追加提交、变基拉取、分支变基、修改提交信息、交互式历史编辑和 reset 前会自动创建 `refs/forkline/recovery/...` 隐藏引用；同一秒内同一分支连续创建同类恢复点时，后续恢复点会追加 `-2`、`-3` 后缀，避免覆盖更早的恢复点；右侧新增“恢复点”页，可查看、恢复或删除恢复点。恢复动作会先再创建一个恢复前恢复点。
- 恢复点管理已增强：右侧“恢复点”页支持搜索、按分支筛选、按动作筛选、显示筛选数量，并支持删除当前筛选结果；恢复点对应提交标题里包含 Tab 时不会被截断，搜索也会使用完整标题；后端新增 `deleteRecoveryPoints`，会先验证所有 ref 都在 `refs/forkline/recovery/...` 下再批量删除。
