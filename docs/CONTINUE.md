...
- 工作区 Diff 精细提交反馈已补齐：未跟踪文件对照会明确提示部分暂存只把选中内容加入索引，其余内容仍留在工作区；按块或按行成功后，最大化窗口会显示本次结果和当前“仍有未暂存 / 已有暂存内容”状态。刷新会保留当前文件、弹窗、横纵滚动位置和已加载 Diff 行数；最后一处改动处理完时也会保留窗口并明确显示无剩余改动。最大化 Diff 放开内部裁剪，使结果提示、行操作栏和新旧版本表头在长 Diff 中保持吸顶。真实浏览器在 320 行未跟踪文件第 260 行附近按行暂存，操作前后 `scrollTop` 均为 `7064.18`，控制台无错误；验收仓库和 `5347` 临时服务已清理。
