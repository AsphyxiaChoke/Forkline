...
- 设置界面已接入：顶部右上“更多”下拉和命令面板都能打开“设置”，目前集中管理本机主题、界面语言、最近仓库、恢复点保留策略，以及侧栏宽度和底部区域高度重置；默认使用中文，可切换为英语并保存在当前浏览器本地，不改仓库配置。界面、弹窗、提示、操作日志、诊断信息、后端错误和相对时间均支持中英双语，仓库路径、分支名、标签、提交信息、作者、引用、提交哈希、命令及原始 Git 输出保持原值。
...
3. 恢复点策略：增加按仓库单独保存的保留策略，以及危险历史操作成功后可选自动整理；执行删除前仍保留候选预览和明确确认。
4. 大仓库体验：以真实压力指标决定提交图和大文件树的增量或虚拟化渲染，补充长时间连续操作回归，不凭假设重写现有渲染。
5. 前端维护性：按实际耦合拆分文件编辑、Diff 工作台、同步和恢复设置等高复杂模块，保持现有脚本加载顺序和 Git 语义。
6. 更新可靠性：补齐下载进度、校验失败提示和失败回退状态，保证快速迭代期间仍可恢复到更新前版本。
