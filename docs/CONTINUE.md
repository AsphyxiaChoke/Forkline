...
- 远端分支右键菜单已接入“删除远端分支”，后端执行 `git push <远端> --delete <分支>` 并随后 `fetch --prune`；无效远端引用不会给出删除入口；后端会拒绝删除 `main` / `master` / `develop` / `dev` / `trunk` 这类远端主干/长期分支；以本地或远端分支为目标的签出、合并、变基、新建分支和创建工作树会比较页面看到的目标分支 SHA，远端分支还会比较真实远端 SHA，如果目标分支已被外部命令移动或真实远端已被别人推进，会拒绝旧页面请求并提示刷新；从 Tag 新建分支或创建工作树会比较页面看到的 Tag 对象 SHA，如果 Tag 已被外部命令移动，会拒绝旧页面请求并提示刷新；设置 upstream 也会比较页面看到的目标远端分支 SHA 和真实远端 SHA；以远端分支为目标的这些操作、设置 upstream 和删除远端分支还会比较页面看到的远端 URL，如果远端 URL 已被外部命令改过，会拒绝旧页面请求并提示刷新，避免使用错误远端分支；删除远端分支前还会比较页面看到的 tracking SHA、当前本地 tracking SHA 和真实远端 SHA，如果同名远端分支已经变化或本地 tracking 已被外部 fetch 更新，会拒绝旧页面请求并提示刷新，避免删除别人新推送的分支或删到错误远端。
...
- Tag 管理已接入：右侧新增“标签”页，显示本地 Tag 列表和详情，支持查看 Tag 提交、复制名称、推送 Tag、删除本地 Tag、删除远端 Tag；Tag 列表会保留完整对象 SHA，写操作会携带页面看到的 SHA；推送或删除远端 Tag 会携带页面看到的远端名和 URL，后端发现远端 URL 已被外部命令改过时会拒绝旧页面请求，避免把 Tag 推到或删到新的远端；Tag 说明里包含 Tab 时不会把说明和对象类型解析错位；Tag 行右键菜单也提供同样的相关动作和 Git 指令提示。
...
- 远端 Tag 操作失败提示已修正：推送或删除远端 Tag 时如果远端 URL / 路径不可读，提示会显示实际远端名，例如 `origin`，不再把 Tag 名误当成远端；删除远端 Tag 前会先用 `ls-remote --tags` 确认该 Tag 仍存在，并比较页面 SHA、本地 Tag SHA 和真实远端 Tag SHA，避免 Git 对不存在 Tag 返回成功 warning 后误报“已删除”，也避免旧页面删除别人新推送的同名 Tag；重复推送远端已有的相同 Tag 会提示无需重复推送；推送同名但不同对象的 Tag 被远端拒绝时，会提示远端已存在同名 Tag，而不是误报“远端名已经存在”。
...
- 提交右键菜单和提交详情面板都已加入“还原”和软 / 混合 / 硬重置入口，并在文案旁标注 `git revert`、`git reset --soft`、`git reset --mixed`、`git reset --hard`。还原会创建反向提交；还原和挑选执行前会校验页面看到的工作区快照，避免旧页面把新提交叠到外部刚出现的改动上；硬重置入口标红且确认文案会提示会丢弃工作区改动；reset 执行前会校验页面看到的工作区快照，避免旧页面把外部刚写入的改动重置掉。
...
- 引用日志恢复已接入：右侧“恢复点”页新增 HEAD reflog 区块，读取 `git log -g HEAD`，可查看 reflog 提交、从某条 reflog 创建 Forkline 恢复点，或在本地分支且工作区干净时恢复到该记录；从 reflog 创建恢复点和恢复到 reflog 记录都会携带页面看到的当前分支和 HEAD，避免旧页面把恢复点归到外部切换后的分支，或把外部切换后的分支硬重置到错误提交；reflog 行右键菜单也提供“查看提交 / 复制 SHA / 创建恢复点 / 恢复到此处”；reflog 字段改用 NUL 分隔，提交标题里包含内部控制分隔符时不会把作者和时间解析错位。
