...
- 顶部路径选择器已恢复并整理布局：路径输入框旁有固定可见的“选择”按钮，可在内置目录弹窗中进入上级目录、跳转输入路径，并通过桌面、下载、文档、用户目录和磁盘根目录快捷入口选择本机 Git 仓库；最近仓库和路径操作按钮已分组，窄宽度会提前换成两行顶栏，避免互相挤压或变形；连续打开多个仓库时只保留最后一次打开结果；有 Git 操作运行时会暂时阻止切换仓库和新的 Git 操作，避免命令执行到错误仓库或多个命令同时修改仓库状态；切换仓库期间旧工作区自动刷新、旧工作区 Diff、旧文件动作刷新、旧储藏详情/动作刷新、旧同步/远端动作刷新、旧分支/Tag/worktree/子模块/补丁动作刷新、旧提交补丁复制/下载结果、旧历史/reset 动作刷新、旧历史重写预览、旧恢复点动作刷新、旧提交详情/文件历史/逐行追踪/比较读取不会覆盖新仓库页面；后端状态、提交详情、补丁、文件历史、逐行追踪、比较、历史重写预览、工作区 Diff 和储藏详情读取会使用请求发起时的仓库路径快照，避免一次响应混入旧仓库和新仓库数据；打开新仓库会清空旧仓库的分支/Tag/mainline 弹窗、储藏详情和右键菜单上下文；克隆或初始化期间如果用户又手动打开其他仓库，完成后的旧自动打开结果不会覆盖当前仓库；目录列表会隐藏 `.git`；快速切换目录时只保留最后选择的目录结果，旧请求或旧错误不会覆盖当前目录。
