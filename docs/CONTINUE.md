...
- 窄屏排版已整体收口：左栏最低 `160px`、右栏最低 `220px`，中心区按视口保留动态最小宽度；提交列表在中心区变窄时依次隐藏作者和 SHA，最窄列配置会扣除行内边距，超长提交标题不会再把时间列或整行推向右侧；提交框、设置卡片、同步操作和文件操作在窄容器中会收缩或换行。提交详情中的“提交操作”“历史编辑”“历史编辑队列”标题和按钮文字以及空队列提示使用专用样式居中；操作按钮在右栏宽于 `300px` 时使用两列，“硬重置”和“丢弃此提交”单独占整行，右栏不超过 `300px` 时恢复单列，其他详情标题不受影响。普通 `.mini-btn` 中的 Git 指令不再并排占宽，页面会把指令补入按钮 `title` 供悬停查看；右键菜单保持直接显示指令，并增加视口高度上限和内部滚动，滚动菜单本身不会触发关闭。
