...
- 当前分支类写操作已加页面快照保护：暂存全部、单文件暂存/取消暂存/丢弃、加入 `.gitignore`、冲突文件取舍、工作区 Diff 按块/按行操作、丢弃全部、创建工作树、初始化/更新/同步子模块、创建/恢复/应用/弹出/删除储藏、从储藏创建分支、提交、追加提交、拉取、变基拉取、推送、安全强推、设置或取消 upstream、应用补丁、合并、分支变基、挑选、还原、reset、恢复点恢复、引用日志恢复、修改提交信息、历史编辑，以及合并/变基/挑选/还原的继续、跳过、中止操作会携带页面看到的当前分支和 HEAD；如果仓库已被外部命令切到其他分支，或当前分支 HEAD 已经移动，后端会拒绝旧页面请求，避免把操作打到错误分支或旧页面之外的新提交；旧脚本缺少快照时也会被拒绝。
...
- 同步页远端仓库管理已接入：显示 `git remote -v` 的 fetch / push URL，支持添加远端、抓取单个远端、修改 URL、删除远端；远端行右键菜单也提供抓取、修改 URL、复制 URL、删除远端，并显示对应 Git 指令；远端名允许 Git 合法的带 `/` 名称，例如 `team/origin`；抓取指定远端、修改 URL 或删除远端会携带页面看到的 fetch/push URL，后端发现远端 URL 已被外部命令改过时会拒绝旧页面请求，避免旧页面覆盖或删除新的远端配置。
...
- Tag 管理已接入：右侧新增“标签”页，显示本地 Tag 列表和详情，支持查看 Tag 提交、复制名称、推送 Tag、删除本地 Tag、删除远端 Tag；Tag 列表会保留完整对象 SHA，写操作会携带页面看到的 SHA；Tag 说明里包含 Tab 时不会把说明和对象类型解析错位；Tag 行右键菜单也提供同样的相关动作和 Git 指令提示。
