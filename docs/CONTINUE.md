...
- 分支整理删除策略：后端新增 `deleteBranches`，批量删除仍逐个执行 `git branch -d`；当前分支、`main` / `master` / `develop` / `dev` / `trunk` 这类主干分支、以及其他 worktree 占用分支会在 UI 禁用；后端也会拒绝删除当前分支和这些主干分支，未完全合并的分支仍由 Git 拦截并返回中文原因。
