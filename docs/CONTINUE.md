...
- 远端分支右键菜单已接入“删除远端分支”，后端执行 `git push <远端> --delete <分支>` 并随后 `fetch --prune`；无效远端引用不会给出删除入口；后端会拒绝删除 `main` / `master` / `develop` / `dev` / `trunk` 这类远端主干/长期分支；删除前会比较本地 tracking SHA 和真实远端 SHA，如果远端同名分支已经变化，会拒绝旧页面请求并提示刷新，避免删除别人新推送的分支。
...
- 分支整理删除策略：后端新增 `deleteBranches`，批量删除仍逐个执行 `git branch -d`；当前分支、`main` / `master` / `develop` / `dev` / `trunk` 这类主干分支、以及其他 worktree 占用分支会在 UI 禁用；后端也会拒绝删除当前分支和删除/重命名这些主干分支，未完全合并的分支仍由 Git 拦截并返回中文原因。
