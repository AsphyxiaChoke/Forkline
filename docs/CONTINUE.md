...
- 图谱分支标签已改为完整显示：移除原先 7 到 8 个字符的省略规则，按完整分支名和所在泳道计算图谱列宽及 SVG 宽度；`feature/visual-history`、`fix/diff-pane-resize`、`release/2.9`、`origin/main` 等标签实测完整显示。
...
- 历史提交完整文件对照已接入：右侧提交“文件”页的变更文件双击后复用现有浮窗和 CodeMirror MergeView，`GET /api/commit-file?sha=&file=&previousFile=` 返回当前提交和第一父提交中的完整文本；重命名使用 `previousFile` 读取旧路径，根提交没有父版本，merge 提交固定与第一父提交比较。新增、删除和一侧不存在时仍保留完整另一侧内容及差异占位。该模式通过独立 `history` 状态严格只读，只保留行号、语法高亮、红绿差异、同步滚动和查找，隐藏保存、替换、暂存按钮及编辑器 Git 右键菜单；再打开工作区文件时会完整恢复可编辑状态。接口继续拒绝非法相对路径、二进制和超过 1 MiB 的文件，并按 UTF-8、UTF-8 BOM、GBK、GB18030 解码。
- 编辑器中间块暂存已改为编辑器专用零上下文 Diff：`/api/worktree-file` 返回 `diffContext = 0`，前端用 MergeView 回调提供的旧版和新版范围共同定位 hunk，并把上下文随 `stageHunk` 传回后端；后端使用同一上下文重新生成补丁，并在零上下文时给 `git apply` 增加 `--unidiff-zero`。因此两个视觉块即使在普通 8 行上下文 Diff 中会合并，点击其中一个也只暂存该块；纯删除块仍可通过旧版范围准确定位。右键打开的最大化工作区 Diff 继续使用原有 8 行上下文，不受影响。
