...
- 当前完整自动回归为 `157/157`：在实时命令与输出、运行状态接口、可取消动作边界、Windows 进程树终止、取消响应与日志状态，以及克隆弹窗和操作日志布局回归基础上，覆盖认证助手系统凭据入口、托管平台识别、历史文件“连线 / 行对齐”切换、大文件与复杂差异轻量路径、旧 MergeView 即时释放、仓库反复切换和真实 Chromium 性能边界。
...
5. 更新可靠性：补齐下载进度、校验失败提示和失败回退状态，保证快速迭代期间仍可恢复到更新前版本。
