...
- 当前分支类写操作已加页面快照保护：暂存全部、单文件暂存/取消暂存/丢弃、加入 `.gitignore`、冲突文件取舍、工作区 Diff 按块/按行操作、丢弃全部、提交、追加提交、拉取、变基拉取、推送、安全强推、设置或取消 upstream、应用补丁、合并、分支变基、挑选、还原、reset、修改提交信息、历史编辑，以及合并/变基/挑选/还原的继续、跳过、中止操作会携带页面看到的当前分支和 HEAD；如果仓库已被外部命令切到其他分支，或当前分支 HEAD 已经移动，后端会拒绝旧页面请求，避免把操作打到错误分支或旧页面之外的新提交；旧脚本缺少快照时也会被拒绝。
...
- Stash 操作已加身份校验：状态接口会返回 stash commit SHA，应用、弹出、删除和从储藏创建分支会把页面看到的 SHA 一起传给后端；如果新的 stash 插入导致 `stash@{n}` 已经指向另一条储藏，后端会拒绝操作并提示刷新，避免删错或弹错储藏。
...
- 自动恢复点已接入：追加提交、变基拉取、分支变基、修改提交信息、交互式历史编辑和 reset 前会自动创建 `refs/forkline/recovery/...` 隐藏引用；同一秒内同一分支连续创建同类恢复点时，后续恢复点会追加 `-2`、`-3` 后缀，避免覆盖更早的恢复点；右侧新增“恢复点”页，可查看、恢复或删除恢复点。恢复动作会先再创建一个恢复前恢复点；恢复和删除会携带页面看到的恢复点 SHA，后端发现同一 ref 已经指向其他提交时会拒绝旧页面请求。
...
- 引用日志恢复已接入：右侧“恢复点”页新增 HEAD reflog 区块，读取 `git log -g HEAD`，可查看 reflog 提交、从某条 reflog 创建 Forkline 恢复点，或在本地分支且工作区干净时恢复到该记录；reflog 行右键菜单也提供“查看提交 / 复制 SHA / 创建恢复点 / 恢复到此处”；reflog 字段改用 NUL 分隔，提交标题里包含内部控制分隔符时不会把作者和时间解析错位。
