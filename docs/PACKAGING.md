...
## v0.4.19 正式发布与本机标准安装验收

- 正式 Release 已完成：[Forkline v0.4.19](https://github.com/AsphyxiaChoke/Forkline/releases/tag/v0.4.19)。Release 为正式版、非草稿、非预发布，中文说明正常展示；不可移动注释标签 `v0.4.19` 的标签对象为 `fc9f8eeb65eacb7d976017cbb778a68b8d2d725f`，解引用后固定指向产品提交 `b719808f7c330edd9825b070e9f38b4eab5e02e2`，后续工作流修正没有移动或重建该标签。
- 安装器工作流 [33732264301](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33732264301) 与便携包工作流 [33724714676](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33724714676) 均为 `success`。安装器工作流在空依赖环境中显式安装 Electron `43.4.1` 运行时后，普通自动测试、Chromium 性能回归、Electron 文件窗口回归、安装器构建、校验和 Release 上传全部通过；构建输入仍为不可变 `v0.4.19` 标签。
- 六个正式附件已重新下载并以 GitHub API digest 为信任根完成本机复核：安装器 `104608860` 字节 / SHA-256 `616c22ce25043cf2e18aefd82179869e8de13b495b4e520eafe85fa0bc657bcb`；blockmap `111640` 字节 / `9100ed3e7399cbc39790bbcb39271057e500eb106a4294832514997c2865477d`；安装器校验文件 `103` 字节 / `55ebcbd8a85a97191cdd88f5757913b26f37b513b0073b66ecd1ee3a3ef90975`；便携 ZIP `36927380` 字节 / `d98e8709f85af9948ca3b46f4361117e9d42693a9e9d39ceb0cca27086bd8187`；便携 ZIP 校验文件 `109` 字节 / `f2d6ddd435cecb15f7bfa1533870a5c2fa4336c1667a73c247f625803eb3a90b`；`latest.yml` `372` 字节 / `4b5b103ba8ac9cb943869373da58b0dab199309d044c9917176672d93f92effe`。
- 两个 `.sha256` 文件分别匹配安装器和便携 ZIP。`latest.yml` 版本为 `0.4.19`，安装器文件名和大小一致，SHA-512 为 `A0/MrtJbDn/Fzax71bjTlOmK4Jc5aqvMqQ6o12H4bEyj6K4MPmQQySz7uOee6WmDkRXQZ/2ptWxDGbjEOOSYnA==`；便携 ZIP 包含 `.git/`、`runtime/node.exe`、`Forkline.cmd`、`start.cmd`、`package.json` 和 `docs/`，继续使用原 Git 快进更新。
- 正式安装器 Authenticode 为 `NotSigned`。Release 说明已明确未知发布者和 SmartScreen 风险，并记录当前用户安装、可选目录、桌面/开始菜单快捷方式、NSIS `electron-updater`、更新前优雅停止 Forkline/Git/SSH 子进程，以及 Web、源码克隆、便携版继续使用 Git 更新的产品边界。
- 正式安装器以静默当前用户模式安装到标准目录 `C:\Users\Administrator\AppData\Local\Programs\Forkline`，退出码为 `0`；`Forkline.exe` 文件/产品版本为 `0.4.19/0.4.19.0`。HKCU 登记显示 `Forkline 0.4.19`，卸载命令指向标准目录；桌面 `D:\桌面\Forkline.lnk` 和开始菜单快捷方式均指向标准安装目录。原有未登记的 `D:\Forkline` v0.4.17 未覆盖、未删除。
...
- 正式 Release 已完成：[Forkline v0.4.20](https://github.com/AsphyxiaChoke/Forkline/releases/tag/v0.4.20)，Release ID 为 `382026154`，为 Latest、正式版、非草稿、非预发布。中文说明明确记录独立历史窗口卡死修复、修复后效果、Electron/Web 功能边界、安装更新方式和未签名风险。
- `v0.4.20` 为注释标签，标签对象为 `d7b9772f783374630047cd74b24edda2775433b4`，解引用后固定指向产品提交 `19dbd2876139402b373c813f02b1dca9bf1532c4`。`v0.4.19` 仍固定指向 `b719808f7c330edd9825b070e9f38b4eab5e02e2`，旧标签未移动。
- 安装器工作流 [33758782496](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33758782496) 与便携包工作流 [33758782162](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33758782162) 均为 `success`，且均使用产品提交 `19dbd2876139402b373c813f02b1dca9bf1532c4`。安装器工作流的普通自动测试、三倍 Chromium 性能回归、Electron 独立窗口回归、构建、校验和上传全部通过。
- 六个正式附件重新下载后的 SHA-256 与 GitHub API digest 全部一致：安装器 `104610046` 字节 / `5f698b4a451d848fb1ee85d0bbdb2616ef8cf417047e537f77fa7a9b2b310538`；blockmap `111739` 字节 / `01ab8a41b83150cefdd09e855b1707709e37318e98390e50f17c6e31a1a8df6d`；安装器校验文件 `103` 字节 / `7515360ac8a13e3f6518031e54f0aa37611d1bb93ecca71cbbfacd27987745fc`；便携 ZIP `36957254` 字节 / `b1424f50f37b9208cbba16ca962661a87a686a7e774d41f76af2f12f4fb3286b`；ZIP 校验文件 `109` 字节 / `9229f722058e4ba8db9b2325b956549b26bbbeadf61e6c45f5bec37a16572e68`；`latest.yml` `372` 字节 / `5736f9123f07f2c00674f8bfe99d754a5aaf69327a43727d19a0ae26f17799c6`。
- 两个 `.sha256` 文件分别匹配安装器和便携 ZIP；`latest.yml` 的版本为 `0.4.20`，安装器文件名、大小和 SHA-512 `gAjHSF4IeO9TBzQFBNse5uwLwCyCjGvT3/+UTZSiaY+CtxO9Ls6/UeItARNXUbiLvUaENNPVX7Y7q23JbyxOmg==` 均匹配。便携 ZIP 共 `332` 个条目，顶层包版本为 `0.4.20`，包含 `.git/`、`runtime/node.exe`、`Forkline.cmd`、`start.cmd`、源码、测试和 `docs/`。
- GitHub 直连大附件在本机停留于 `0` 字节后已终止，仅对两条 `curl` 下载命令使用本机 `127.0.0.1:7897` 代理；没有修改全局代理配置，最终信任依据仍是 GitHub API digest。审计目录为 `C:\Users\Administrator\AppData\Local\Temp\forkline-v0.4.20-release-audit-20260903-2110`。
- 正式安装器 Authenticode 为 `NotSigned`。发布说明已标注未知发布者与 SmartScreen 风险；安装器继续按当前用户安装、允许选择目录、默认创建桌面和开始菜单快捷方式，NSIS 安装版继续使用 `electron-updater`，更新前优雅停止 Forkline 后台服务及 Git/SSH 子进程。
...
- 正式 Release 已完成：[Forkline v0.4.21](https://github.com/AsphyxiaChoke/Forkline/releases/tag/v0.4.21)，Release ID 为 `382447217`，为 Latest、正式版、非草稿、非预发布。中文说明正常展示，完整记录 MergeView、快速滚轮/滚动条、任务栏、最大化、安装更新边界和未签名风险。
- `v0.4.21` 为注释标签，标签对象为 `1a3d9179482af6684adb07b8d580f3a45ec8d03b`，解引用到产品提交 `a76b5d4991c1393354717262bf6b7dfd9ab9f5e0`；`v0.4.20` 仍固定解引用到 `19dbd2876139402b373c813f02b1dca9bf1532c4`。
- 便携包工作流 [33830924733](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33830924733) 为 `success`。安装器工作流 [33830924570](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33830924570) 首次在 GitHub Runner 整体变慢时触发无关提交图谱 `92628 px` 时序断言，第二次通过 Chromium 后等待普通独立编辑器加载超时；第三次仍使用同一不可变标签，完整自动测试、三倍 Chromium、Electron `2/2`、安装器构建、校验和附件上传全部通过，最终结论为 `success`。
- 六个正式附件的本机 SHA-256 全部匹配 GitHub API digest：安装器 `104609165` 字节 / `7d3409b22450636f38a23939d40643f0de3d1bb8fc2bb4d0290f381cfffd5b20`；blockmap `111730` 字节 / `19e62ba8f9cdb986ce40ded100ce2a8aa4a9417cb0bfe5ae3198b4c8c36b124c`；安装器校验文件 `103` 字节 / `01c218d444d207f2488628131e258407e4dc7c8b098931901a77e17801300faa`；便携 ZIP `36972605` 字节 / `49fbc448114895b984a8123b3684a67333a3b7b262387986853a35bf6aee84ae`；ZIP 校验文件 `109` 字节 / `e773fba19de9f8f2a5841162a8620dc6d0ea951361f98315fd1afd07109f21be`；`latest.yml` `372` 字节 / `5b33eca5d67192d62697a4cf2cf34eb90e270ee7ee06282f31ab270c79a9d49c`。
- 两个 `.sha256` 内容分别匹配正式 EXE 和便携 ZIP；`latest.yml` 的版本、文件名、大小和 SHA-512 全部匹配。便携 ZIP 共 `332` 个条目，包版本为 `0.4.21`，包含 `.git/`、`runtime/node.exe`、`Forkline.cmd`、`start.cmd` 和 `docs/`。正式安装器 Authenticode 为 `NotSigned`。
- GitHub 正式安装器隔离安装退出码为 `0`，EXE 文件/产品版本为 `0.4.21/0.4.21.0`；安装后 `app.asar` 版本为 `0.4.21`，直接确认包含任务栏、捕获阶段滚轮、独立窗口铺满和完整 MergeView 修复。隔离安装版 Electron 回归为 `2/2`，9 条滚轮来源的最大向上跳变、最终偏差和长任务均为 `0`；历史窗口连续拖动 `8` 轮峰值约 `309.0 MiB`、停止后约 `308.3 MiB`，DOM 与绘图层稳定。隔离卸载退出码为 `0`，目录和登记均已删除。
...
- Release 首次触发的安装器 Run `33886532954` 连续三次停在 Chromium 回归，产品普通测试均已通过且未生成安装器附件。第一次和第三次均显示实际页面已挂载 `4000` 个文件行、滚动从 `0` 到约 `19215 px` 且无回弹，但“分散目录”夹具的虚拟块计数变为 `0`、页面高度从 `248061` 变为 `123293`；第二次是共享 Runner 的设置面板首次载入 `3256.5 ms`，超过三倍容差 `3000 ms`。三次失败点和本次 MergeView 修复无关。
- 根因是慢 Runner 在物理滚轮循环期间触发后台工作区轮询，用真实仓库的 4000 文件覆盖了合成的 4000 分散目录夹具。修正测试在该夹具期间暂时让 `document.hasFocus()` 返回 `false`，结束后恢复原函数，沿用相邻压力场景的现有隔离方式；没有放宽产品性能门限或断言。安装器工作流仅在手动重建 `v0.4.22` 时借用默认分支修正后的浏览器测试，随后恢复不可变标签中的测试文件，再构建产品。
- 修正后安装器工作流契约 `2/2`、真实 Chromium `1/1` 通过；分散目录保持 `4000` 行、`40` 个虚拟块，快速滚轮高度保持 `248061 px`、最大向上跳变 `0 px`。正式 v0.4.22 标签仍固定在产品提交 `353c9e865d09908224b9d2f30b389eb57aa06d86`。
...
## v0.4.22 正式发布、附件验真和标准安装验收

- 正式中文 Release 已发布并成为 Latest：[Forkline v0.4.22](https://github.com/AsphyxiaChoke/Forkline/releases/tag/v0.4.22)，Release ID `382809557`，非草稿、非预发布。注释标签对象 `8c9367f4e04b500bf603ae929b7042630a205fcf` 固定解引用到产品提交 `353c9e865d09908224b9d2f30b389eb57aa06d86`；后续测试/工作流提交 `0969f0c917c17d02a631d5b23c937c97d30bd143` 在 `main` 上，没有移动产品标签。
- 便携包工作流 [33886533217](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33886533217) 为 `success`。安装器首次 Release Run `33886532954` 的三次共享 Runner 失败均未上传错误附件；采用隔离夹具的手动安装器 Run [33888762128](https://github.com/AsphyxiaChoke/Forkline/actions/runs/33888762128) 为 `success`，普通测试 `408/408`、Chromium `1/1`、Electron `2/2`、恢复标签测试、构建和上传全部通过。
- 六个正式附件的本机 SHA-256 全部匹配 GitHub API digest：安装器 `104611043` 字节 / `2bd0de7b22fadb7dd46e01f6c2a06d916bbe33d7c5c04c50787aa52c39af7664`；blockmap `111588` 字节 / `30c25e2a7f944f1c69d6d6b2f1fe1e33efa8583b657079d19962389f1e9fece7`；安装器校验文件 `103` 字节 / `10539c0f547c2998b6fe438501f12e468e05f200d9f7d393e3fcf5996724476b`；便携 ZIP `36999742` 字节 / `0df7b41fcd89d9b3d4881133ab48f33e9802203b4a874c0765105a10dca08a7a`；ZIP 校验文件 `109` 字节 / `bd1121c1c6db043dd1b0a2eee9e4de9d4b97be9b87e8588e69d8f65866d693a2`；`latest.yml` `372` 字节 / `b2f85bf82474a2f667a03cefb85bc5fa5cf8d84eb0db6382c14b0eb05d118380`。
- 两个 `.sha256` 内容分别匹配正式 EXE 和便携 ZIP；`latest.yml` 的版本、文件名、大小和 SHA-512 `xA+PkjI4LdJMx1HR5XZAdrosfvZn7I1bzWfjGvsDmONfR4KP+57C34Da8Kj3EbCjJmfXmR0AIQs7fC9Xn8SolA==` 全部匹配。正式安装器 Authenticode 为 `NotSigned`，中文发布说明已明确未知发布者和 SmartScreen 风险。
- 便携 ZIP 共 `332` 个条目，包版本为 `0.4.22`；包含浅层 `.git`、`main`、官方 `origin`、`runtime/node.exe`、`Forkline.cmd`、`start.cmd` 和文档。浅层 `main` 固定在产品提交 `353c9e865d09908224b9d2f30b389eb57aa06d86`，继续使用原 Git 快进更新。
...
v0.4.24 首次安装器任务在原生作者列拖动检查失败。独立 Headless Edge 复现确认：重置布局后鼠标尚未移动，读取到旧作者分隔线位置；鼠标移动触发布局稳定后，同一坐标已经位于时间单元格，因此没有开始拖动。

修正后的测试先移动指针，再等待两帧、重新读取目标位置并校验命中分隔线，然后按原有 48px 拖动、60px 松手位置和时间门槛验收。没有修改产品代码或放宽断言。

手动执行安装器工作流并指定 `tag=v0.4.24` 时，仅从工作流提交借用 `tests/helpers/layout-drag.js`；测试后恢复为不可变标签中的文件，再构建发布附件。产品源码仍固定为 v0.4.24，已有标签和已上传附件不覆盖。
