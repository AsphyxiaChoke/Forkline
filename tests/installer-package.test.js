...
test("Windows installer is per-user, assisted, x64, and creates standard shortcuts", () => {
  assert.equal(pkg.version, "0.4.19");
...
test("installer release workflow publishes updater metadata and checksums", () => {
  ...
  assert.match(workflow, /name:\s*Use corrected release Electron regression harness[\s\S]*?github\.event_name == 'workflow_dispatch' && steps\.release\.outputs\.tag == 'v0\.4\.19'[\s\S]*?git restore --source='\$\{\{ github\.sha \}\}' --worktree -- tests\/electron-file-editor-performance\.test\.js/);
  ...
  assert.match(workflow, /name:\s*Run Electron editor regression[\s\S]*?TcpListener[\s\S]*?FORKLINE_ELECTRON_CDP_PORT[\s\S]*?node tests\/electron-file-editor-performance\.test\.js/);
  ...
  assert.match(workflow, /name:\s*Restore release-tag Electron regression harness[\s\S]*?always\(\)[\s\S]*?steps\.release\.outputs\.tag == 'v0\.4\.19'[\s\S]*?git restore --worktree -- tests\/electron-file-editor-performance\.test\.js/);
