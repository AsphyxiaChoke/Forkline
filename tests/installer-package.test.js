...
test("Windows installer is per-user, assisted, x64, and creates standard shortcuts", () => {
  assert.equal(pkg.version, "0.4.24");
...
test("v0.4.24 manual retry restores the tag after borrowing only corrected test harnesses", () => {
  assert.match(workflow, /name: Use corrected v0\.4\.24 native drag harness\s+if: github\.event_name == 'workflow_dispatch' && steps\.release\.outputs\.tag == 'v0\.4\.24'\s+shell: pwsh\s+run: git restore --source='\$\{\{ github\.sha \}\}' --worktree -- tests\/helpers\/layout-drag\.js tests\/browser-performance\.test\.js/);
  assert.match(workflow, /name: Restore v0\.4\.24 native drag harness\s+if: always\(\) && github\.event_name == 'workflow_dispatch' && steps\.release\.outputs\.tag == 'v0\.4\.24'\s+shell: pwsh\s+run: git restore --worktree -- tests\/helpers\/layout-drag\.js tests\/browser-performance\.test\.js/);
...
test("installer release workflow publishes updater metadata and checksums", () => {
  ...
  assert.match(workflow, /name:\s*Use corrected release browser regression harness[\s\S]*?github\.event_name == 'workflow_dispatch' && \(steps\.release\.outputs\.tag == 'v0\.4\.1' \|\| steps\.release\.outputs\.tag == 'v0\.4\.3' \|\| steps\.release\.outputs\.tag == 'v0\.4\.22'\)[\s\S]*?git restore --source='\$\{\{ github\.sha \}\}' --worktree -- tests\/browser-performance\.test\.js/);
  ...
  assert.match(workflow, /name:\s*Restore release-tag browser regression harness[\s\S]*?always\(\)[\s\S]*?steps\.release\.outputs\.tag == 'v0\.4\.22'[\s\S]*?git restore --worktree -- tests\/browser-performance\.test\.js/);
